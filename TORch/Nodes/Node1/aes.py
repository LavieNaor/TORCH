...
def sub_bytes(s):
    """
    The function replace each bytes according to the s-box table
    :param s: The chunk
    :type s: list
    :return: None
    """
...
def inv_sub_bytes(s):
    """
    The function replace each bytes according to the inv-s-box table
    :param s: The chunk
    :type s: list
    :return: None
    """
...
def get_key_iv(password, salt, workload=100000):
    """
    Stretches the password and extracts an AES key, and an AES initialization vector.
    """
    stretched = pbkdf2_hmac('sha256', password, salt, workload, AES_KEY_SIZE + IV_SIZE)
...
def encrypt(key, plaintext, workload=100000):
    """
    Encrypts `plaintext` with `key` using AES-128 and PBKDF2 to stretch the given key.
    """
...
def decrypt(key, ciphertext, workload=100000):
    """
    Decrypts `ciphertext` with `key` using AES-128 and PBKDF2 to stretch the given key.
    """
